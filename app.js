...
async function teacherAddStudent() {
  ...
  if (!name) return showToast('⚠️ Nome é obrigatório');
  try {
    const r = await api('POST','/api/students',{name,level,lang,cpf,email,whatsapp,payday,price});
...
function lessonItemHTML(l, isTeacher) {
  ...
  return `<div class="lesson-item">
    ...
    <div class="li-info">
      ...
      ${l.meetLink?`<a class="meet-link" href="${l.meetLink}" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()">🎥 Entrar no Google Meet</a>`:''}
    </div>
